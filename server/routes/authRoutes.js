...
//     }
// // Ensure this matches your model's property name
//     return res.status(200).json(row); // Return the found document
//   } catch (err) {
//     console.error(err); // Log the error for debugging
//     return res.status(500).json({ message: "Server error" });
//   }
// });
// router.post("/update/statics1", async (req, res) => {
//   const { currentaddress,currentnetwork } = req.body;
//   try {
//     const id ='6742b690622c133cc0a6637a';
//     const row = await StaticVaraible.findById(id); // Assuming 'id' is the _id of the order
   
//     row.address = currentaddress; // Set the order status to "Complete"
//     row.network = currentnetwork; // Set the order status to "Complete"
//     await row.save();
